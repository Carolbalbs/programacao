...
int main() {
	...
	while(continuar) {
		...
		while (true) {
			std::cout << "1 - Inserir Valor na Árvore\n2 - Verificar Número na Árvore\n3 - Excluir Número na Árvore\n4 - Mostrar TODOS os Valores da Árvore\n5 - Encerrar" << std::endl;
			std::cin >> nEsc;
			if (nEsc == 1 || nEsc == 2 || nEsc == 3 || nEsc == 4 || nEsc == 5)
				break;
		...
		switch(nEsc) {
			...
			case 3:
				...
				break;
//função fora e chama dentro do case
			case 4:
        int main(){
...
//correr lista numa estrutura de repetição for pra cada indice da lista dá um push
          for(int nEsc)
//percorrer indices
          lista.push_front(nEsc)
          cout << "Tamanho da lista: " << lista.size() << "\n\n";

//apagar
          tam = lista.size();
          for(int nEsc=0; nEsc<tam; nEsc++){
            cout << lista.front() << "\n";
            lista.pop_front()
          return 0;
  }
...
//apagar
				std::cout << "Nós " << n << " adicionados à Árvore !" << std::endl;
				break;
